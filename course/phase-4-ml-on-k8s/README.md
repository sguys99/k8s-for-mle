...
| 02 | KServe InferenceService | sentiment 분류 모델을 KServe `InferenceService` 로 마이그레이션, autoscaling / canary / explainer 의 표준 |
| 03 | vLLM LLM Serving ⭐ GPU | `microsoft/phi-2` 또는 `Qwen/Qwen2.5-1.5B-Instruct` 를 OpenAI 호환 API 로 서빙, KV cache / paged attention 의 GPU 메모리 효과 |
| 04 | Argo Workflows | DAG 워크플로 기초, RAG 인덱싱 파이프라인 프로토타입 (캡스톤의 발판) |
| 05 | Distributed Training Intro | KubeRay vs Kubeflow Training Operator 의 디자인 차이, 짧은 분산 학습 실습 |
...
➡️ Phase 4 모두 완료 후: [⭐ Capstone — RAG 챗봇 + LLM 서빙 종합 프로젝트](../capstone-rag-llm-serving/) (작성 예정)
