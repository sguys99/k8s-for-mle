...
Day 5 시점에서는 **`practice/rag_app/` 의 6 개 코드 모듈만 존재**하고 클러스터 매니페스트는 아직 없습니다. Day 6 에서 다음 4 개 매니페스트를 추가해 본 코드를 클러스터로 옮깁니다.

- `manifests/30-rag-api-deployment.yaml` — Deployment(replicas=2), 환경변수는 ConfigMap/Secret 에서 주입, livenessProbe `/healthz` + readinessProbe `/ready`
- `manifests/31-rag-api-service.yaml` — ClusterIP Service `rag-api.rag-llm.svc:8001`
- `manifests/32-rag-api-configmap.yaml` — `.env.example` 의 6 개 환경변수를 ConfigMap 으로 외재화
- `manifests/33-rag-api-secret.yaml` — (옵션) HF 토큰 재사용

Day 5 단계의 학습 목표는 *코드 모듈 분리 + 로컬 검증* 까지이므로 본 절의 매니페스트 해설은 Day 6 으로 미룹니다. Day 5 의 `Dockerfile` 과 `.env.example` 은 Day 6 에서 ConfigMap/Secret 매니페스트로 전환되는 *원본 명세서* 역할을 합니다.
...
<!-- 캡스톤 진행 중 발견 시 누적 추가합니다. 현재 Day 1(Qdrant/StatefulSet 3건) + Day 2(인덱싱 3건) + Day 3(Argo 3건) + Day 4(vLLM/GPU 3건) + Day 5(RAG API 3건) = 15건. -->
