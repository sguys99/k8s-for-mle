...
| 축 | 핵심 메트릭 | 출처 | HPA 연동 |
|---|---|---|---|
| RAG API | `rag_chat_total{status}` · `rag_chat_latency_seconds` · `rag_retrieve_latency_seconds` · `rag_llm_latency_seconds` (Day 5 시점 라벨 — Day 7 ServiceMonitor 가 그대로 수집) | RAG API `/metrics` | RPS 기반 HPA (Day 8) |
...
- [`../manifests/30-rag-api-deployment.yaml`](../manifests/30-rag-api-deployment.yaml) — RAG API Deployment (replicas=2, env 6 종 직접 박기, `/healthz` liveness + `/ready` readiness/startup, hf-cache emptyDir 1Gi, `imagePullPolicy: IfNotPresent` + tag 핀 `:0.1.0`, RollingUpdate maxSurge=1 maxUnavailable=0)
