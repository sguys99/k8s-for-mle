...
| [03](./03-ingress/) | Ingress | `minikube addons enable ingress` 로 nginx-ingress-controller 설치, `/v1/sentiment` 경로 기반 라우팅 + `rewrite-target`, ML 모델 응답 지연 대응 annotation(`proxy-read-timeout`, `proxy-body-size`, `proxy-buffering`), `minikube tunnel`/`port-forward` 외부 접근, pathType / ingressClassName 의도적 실패 시연 |
